...
load_language_ru () {
	...
	msg_checkFusePermission="Проверьте: вы должны быть в группе fuse. Наберите в консоли: \"sudo adduser $USER fuse\""
	...
	msg_change_password_error="Проблемы изменения пароля \"${sourceDirName}\"!\nЭто не зашифрованный каталог EncFS."
	...
	msg_config_cryptMode="Предпочтительный режим шифрования:"
	msg_config_cryptMode_standard="Стандартный, безопасный и более совместимый."
	msg_config_cryptMode_paranoia="Паранойя, более безопасный, но менее совместимый."
	msg_config_autoUnmount="Автоматически размонтировать каталог?"
	msg_config_autoUnmount_fm="Да, после закрытия файлового менеджера."
	msg_config_autoUnmount_no="Нет, я делаю это вручную."
	msg_config_saved="Настройки сохранены."
	msg_save_config_failed="Настройки не сохранены!"
}
...
encMount () {
	PASSWORD=$("${KDIALOG}" --title "${msg_askPassword_title}" --password "${msg_askPassword_text}") || {
		"${KDIALOG}" --title "${msg_warning_title}" --passivepopup "${msg_mount_abort}" 5
		exit
		}

	# Check destination directory
	[ -d "${destDir}" ] && { "${KDIALOG}" --title "${msg_error_title}" --sorry "${msg_checkDestDir}"; exit; }

	# Make destination directory
	mkdir "${destDir}" || { "${KDIALOG}" --title "${msg_error_title}" --error "${msg_mkDestDir}"; exit; }
	...
	echo "${cryptMode}" | encfs --extpass="$0 ${lang} askPassword ${sourceDir} '${PASSWORD}'" "${sourceDir}" "${destDir}" || {
		"${KDIALOG}" --title "${msg_error_title}" --error "${msg_mountError}"
		rm -r "${destDir}"
		exit
		}
...
encPasswdChange () {
	...
	konsole --title "${msg_change_password_konsole_title}" -e encfsctl passwd "${sourceDir}" || \
	"${KDIALOG}" --title "${msg_error_title}" --error "${msg_change_password_error}"
}
